...
repo_home=/home/${USER}/git/hog
src_path=${repo_home}/data

# input/output data directory path on the scratch disk of the node
dest_path=${SCRATCH_HOME}/hog/data
mkdir -p ${dest_path}  # make it if required

output_path=${SCRATCH_HOME}/hog/output
mkdir -p ${output_path}  # make it if required
...
# Check that image data is not in old format (if it is then delete it)
if [ -d ${dest_path}/train/1 ]
then
    rm -rf ${dest_path}/train
    rm -rf ${dest_path}/val
    rm -rf ${dest_path}/annotated/train
    rm -rf ${dest_path}/annotated/val
    rm -rf ${dest_path}/annotated/test
    rm ${dest_path}/*.zip
    rm ${dest_path}/annotated/*.zip
fi


rsync --archive --update --compress --progress ${src_path}/ ${dest_path}
unzip -n ${dest_path}/train.zip -d ${dest_path}
unzip -n ${dest_path}/val.zip -d ${dest_path}
unzip -n ${dest_path}/annotated/train.zip -d ${dest_path}/annotated
unzip -n ${dest_path}/annotated/val.zip -d ${dest_path}/annotated
unzip -n ${dest_path}/annotated/test.zip -d ${dest_path}/annotated
...
src_path=${SCRATCH_HOME}/hog/output
dest_path=${repo_home}/output
rsync --archive --update --compress --progress ${src_path}/ ${dest_path}
